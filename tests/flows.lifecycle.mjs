...
await (async () => {
  ...
  // admin login
  const ajar = makeJar();
  const alogin = await api("POST", "/api/auth/login", { idNumber: "ahmad.alfailakawi@test.kw", password: "A97852900" }, { jar: ajar, deviceToken: "admin-dev" });
...
await (async () => {
  const ajar = makeJar();
  await api("POST", "/api/auth/login", { idNumber: "ahmad.alfailakawi@test.kw", password: "A97852900" }, { jar: ajar, deviceToken: "admin-dev" });
