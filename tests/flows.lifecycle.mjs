...
const tjar = makeJar();
const login = await api("POST", "/api/auth/login", { idNumber: AA, password: "A97852900" }, { jar: tjar, deviceToken: "teacher-A-dev" });
...
await (async () => {
  ...
  // admin login
  const ajar = makeJar();
  const alogin = await api("POST", "/api/auth/login", { idNumber: "ah.alfailakawi@paaet.edu.kw", password: "A97852900" }, { jar: ajar, deviceToken: "admin-dev" });
...
await (async () => {
  const ajar = makeJar();
  await api("POST", "/api/auth/login", { idNumber: "ah.alfailakawi@paaet.edu.kw", password: "A97852900" }, { jar: ajar, deviceToken: "admin-dev" });
